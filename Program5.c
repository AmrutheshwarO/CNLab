...
void main() {
    ...
    printf("Enter a message to be sent (Max 125 characters).\n");
    ...
    printf("Enter the divisor (generator) of 17 bits.\n");
    scanf("%s", div);
    divide(augdw, div, rem);
    strcpy(cw, augdw);
    strcpy(&cw[strlen(augdw)-16], rem);
    strcpy(rcw, cw);
    printf("Enter the number of errors to be introduced during transimission.\n");
    scanf("%d", &e);
    srand(time(0));
    for(j = 0; j < e; j++)  {
        k = rand()%strlen(rcw)-1;
    ...
    if(strcmp(rem, "0000000000000000") != 0)
        printf("Erraneous transmission detected.\n");
